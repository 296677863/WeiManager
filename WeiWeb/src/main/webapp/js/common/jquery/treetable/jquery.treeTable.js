        ...
        function formatNode(tr) {
            ...
            if (cMap[id] == 0) {
                ...
                if (!$cur.attr('isFirstOne')) {
                	$cur.find("td").css("border-top","0px");
                    var $preSpan = $('#' + $cur.attr('prevId'), $treeTable).children("td").eq(1).find('.prev_sp').clone();
                } else {
                	$cur.find("td").css("border-top","0px");
                    var $parent = $('#' + cMap[id], $treeTable);
                    //没有上一个兄弟节点，则使用父节点的prev_sp
                    var $preSpan = $parent.children("td").eq(1).find('.prev_sp').clone();
            ...
            var $td = $cur.children("td").eq(1);
            if(($cur.attr("depth") == "1" && !$cur.attr("haschild"))){
